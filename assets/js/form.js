...
document.getElementById("registerForm").addEventListener("submit", function(event) {
    ...
    // Use setTimeout to allow the modal to open after the alert
    setTimeout(function() {
        let modalSignin = new bootstrap.Modal(document.getElementById('modalSignin'));
        modalSignin.show();
    }, 0);  // Delay of 0 ensures the modal is triggered right after the alert is closed
});
...
document.addEventListener('DOMContentLoaded', function () {
      ...
      // Check if there's a logged-in user and display their username
      if (loggedInUser && loggedInUser.username) {
        document.getElementById("greeting").innerText = `Hi, ${loggedInUser.username}!`;
      } else {
